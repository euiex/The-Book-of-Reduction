...
# 4. Бесконечная Мерность и Базис \(\{\lvert e_i \rangle\}_{i=1}^{\infty}\)
...
> Что это значит?  
> "Бесконечно-мерно" означает, что для его полного описания требуется бесконечное количество "независимых направлений" или "измерений".  
> "Сепарабельность" означает, что у этого бесконечномерного пространства есть счётный базис — набор из бесконечного, но перечисляемого числа элементарных векторов \(\lvert e_1 \rangle, \lvert e_2 \rangle, \lvert e_3 \rangle, \ldots\).

Свойства базиса:
- Они взаимно ортогональны \(\left(\langle e_i \mid e_j \rangle = 0 \text{ при } i \neq j \right)\), то есть "независимы" друг от друга.
- Имеют единичную норму \(\left(\| e_i \| = 1 \right)\), то есть являются "чистыми" элементарными единицами.
- Любое состояние \(\lvert \psi \rangle\) в \(H_{\text{Халал}}\) может быть выражено как линейная комбинация этих базисных векторов:
  
  \[
  \lvert \psi \rangle = \sum_{i=1}^{\infty} c_i \lvert e_i \rangle, \quad c_i \in \mathbb{C}, \quad \sum_{i=1}^{\infty} |c_i|^2 < \infty
  \]
  
  Здесь \(c_i\) — это комплексная амплитуда, или "вклад", каждого элементарного базисного вектора в общее состояние.

Аналогия:  
Это как аналог того, как любая сложная мелодия может быть выражена как сумма простых нот. Каждое \(\lvert e_i \rangle\) — это как фундаментальный "звук", "буква" или "смысловой атом", а \(c_i\) — его амплитуда, то есть вклад в общее состояние \(\Psi\).

Каббалистическая Интерпретация:  
Бесконечная мерность отражает неисчерпаемость Божественной потенциальности. Базисные векторы \(\{\lvert e_i \rangle\}\) могут соответствовать фундаментальным, неделимым "элементам" потенциальности, которые мы соотносим с "Буквами" (Отиот). Сепарабельность является условием "внятности мира": она позволяет "сформулировать" реальность на счётном множестве различений. Без неё Халал был бы полным хаосом; с ней — это пространство, где могут **потенциально** возникать язык, структура и творение.
